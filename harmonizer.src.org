...
#+call: reset-working-directory()
#+call: tangle-buffer()
#+call: generate-package-docs()
...
#+BEGIN_SRC R :tangle .Rprofile :noweb tangle
  ## --------------------------------------------------------------------------------
...
#+BEGIN_SRC R
  ...
  ##' @param update Update values in `col` column. Default is FALSE. If set `append`, `name` and `name_suffix` are ignored. Also if set the default for `return_all_data_cols` will be set to TRUE.
  ...
  ##' @param name_suffix If `name` is not set the use this as suffi (default is "harmonized"). If the name with the same suffi already edataists in `return_data_cols` it will add counter at the end to avoid variables with the same names.
  ...
  ##' @param return_data_cols If data is table, set the columns to cbind to the result table. Default is cbind all but the original (unharmonized) column (col).
  ##' @param return_all_data_cols Whether to bind all columns in `data`. Defaults depends on values of `update` and `append`. If either is set then defaut values is TRUE otherwise FALSE. If set to TRUE by user the return_data_cols is ignored.
  ##'
  ...
  inset_vector <- function(data
                         ...
                         , return_all_data_cols = append | update ) {
  
      ## inset filtered rows (this makes list if vector is list)
      vector %<>% inset(ommitted_rows_values, rows, .)
      ## inset vector to data
      if(is.atomic(data) & isFALSE(return_data_cols == 1)) {
          vector
      } else if(update) {
          data %>%
              harmonize_defactor(conv2dt = "all") %>% 
              inset2(col, value = vector) %>% 
              edatatract(., ,return_data_cols, with = FALSE)
      } else if(isTRUE(return_data_cols == 0)) {
          vector
      } else {
          ## set harmonized name
          ...
          name %<>%
              harmonize_eval_if_empty(
                  harmonize.add.suffi(data.names[col]
                                    , name_suffix
                                    , data.names[return_data_cols])) %>%
              make.names
          ...
          vector %>%
              data.table %>%          # should make one column even if inset is list
              set_names(name) %>%
              harmonize.data.cbind(data[, return_data_cols, with = FALSE], append)
      }
  }
