...
#+BEGIN_SRC emacs-lisp
  (save-window-excursion
    (let* ((dots-get-dots-block-name "get_dots")
           ...
           ;; assume running in harmonizer.src.org
           (harmonizer-r-pkg-path (file-name-directory (buffer-file-name)))
           (tangle-to-file (concat harmonizer-r-pkg-path "R/get_dots.r")))
...
#+BEGIN_SRC R :tangle inst/tinytest/test_get_harmonize_options.r
  expect_equal(get_harmonize_options(), as.list(formals("harmonize_options")))
  
  harmonize_test <- function (x, ...) get_harmonize_options()
  
  ## should update defauls
  expect_equal(harmonize_test()$col, 1)
  expect_equal(harmonize_test(col = 2)$col, 2)
  
  harmonize_test <- \(x, col = 2, ...) get_harmonize_options()
  ...
  harmonize_test_1 <- \() get_harmonize_options()
  ...
  harmonize_test <- \(x, col = 123) get_harmonize_options()
  expect_equal(harmonize_test()$col, 1)
  
  harmonize_test <- \(col = 123, ...) get_harmonize_options()
  expect_equal(harmonize_test()$col, 1)
  ...
  harmonize_test <- \(x, col = 123, ...) {
      harmonize_ <- \(x, ...) get_harmonize_options()
      harmonize_() 
  }
  ...
  harmonize_test <- \(x, col = 123, ...) {
      harmonize <- \(x, ...) get_harmonize_options()
      harmonize() 
  }
  expect_equal(harmonize_test()$col, 1)
#+END_SRC
