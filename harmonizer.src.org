...
#+BEGIN_SRC R
    org.names.test %>%
        harmonize(procedures = list(
                    ...
                    , "harmonize_toupper"
                    , list("harmonize_squish_spaces", wrap_in_spaces = TRUE)
...
:ID:       org:i762gum0fqi0
:END:
#+BEGIN_SRC R :tangle R/harmonize_make_procedures_list.r
...
#+BEGIN_SRC R :tangle R/update_formals.r
...
#+BEGIN_SRC R
  ...
  harmonize_add_suffix("x.pro.11", "pro"
...
:ID:       org:77wbsm80daj0
:END:
#+BEGIN_SRC R :tangle R/handel_target_naming_indexing.r
  ## naming col
...
#+BEGIN_SRC R :tangle no
  ...
  ##' @param update Update values in `col` column. Default is FALSE. If set `append`, `name` and `output_col_name` are ignored. Also if set the default for `select_all_data_cols` will be set to TRUE.
  ##' @param rows Logical vector to filter records of interest. Default is NULL which means do not filter records
  ##' @param omitted_rows_value If `rows` is set merge these values to the results. It should be a vector of length 1 or `nrow(data)`. If the value is NULL (default) then use original values of `col`.
  ##' @param append If set then put `vector` as the last instead of first vector/column. Default is FALSE.
  ##' @param output_col_name If `name` is not set the use this as suffix (default is "harmonized"). If the name with the same suffix already edataists in `select_data_cols` it will add counter at the end to avoid variables with the same names.
  ##' @param name Use this name for the first column in results (harmonized names). Default is NULL, which means that either name_if_data_atomic if data is vector or original col name will be used with `output_col_name` at the end.
  ...
  inset_vector <- function(data
      ...
      vector %<>% inset(omitted_rows_value, rows, .)
  ...
  check_args_for_inset_vector <- function(env = parent.frame()) {
      evalq({
          ...
          harmonize_is_ok_type(vector
                             ...
                             , type = c("atomic", "list"))
          ## - check omitted_rows_value
          if(!harmonize_is_ok_type(omitted_rows_value
                                 , x.length = c(1, harmonize_data_length(x))
                                 , type = "atomic")) {
              omitted_rows_value <- harmonize_data_get_col(x, col)
          } else if(length(omitted_rows_value) == 1) {
              omitted_rows_value <-
                  harmonize_defactor(omitted_rows_value) |>
                  rep(harmonize_data_length(x))
          } else {
              omitted_rows_value <- harmonize_defactor(omitted_rows_value)
          }
          ## - check select_cols
          ...
          if(!update) {
              ...
              harmonize_is_ok_type(output_col_name
                                 , x.length = 1
                                 , type = "character"
                                 , allow.na = FALSE, allow.null = FALSE)
...
#+BEGIN_SRC R :tangle R/0_replace_patterns.r
...
#+BEGIN_SRC R :tangle R/0_detect_patterns.r
...
#+NAME: cockburn.procedures_table
| no | message                                                     | function.call                                    |
|----+-------------------------------------------------------------+--------------------------------------------------|
|    | *Prepossessing*                                             |                                                  |
|  1 | Cleaning spaces                                             | 'harmonize_squish_spaces', wrap_in_spaces = TRUE |
...
#+call: make-rds-patterns-table(data = cockburn.procedures_table, data.name = "cockburn.procedures_table", remove.single.quotes = "FALSE")
