...
#+BEGIN_SRC R :tangle no
  ...
  ##' @param update Update values in `col` column. Default is FALSE. If set `append`, `name` and `output_name` are ignored. Also if set the default for `select_all_data_cols` will be set to TRUE.
  ##' @param rows Logical vector to filter records of interest. Default is NULL which means do not filter records
  ...
  ##' @param output_name If `name` is not set the use this as suffix (default is "harmonized"). If the name with the same suffix already edataists in `select_data_cols` it will add counter at the end to avoid variables with the same names.
  ##' @param name Use this name for the first column in results (harmonized names). Default is NULL, which means that either name_if_data_atomic if data is vector or original col name will be used with `output_name` at the end.
  ...
  check_args_for_inset_vector <- function(env = parent.frame()) {
      evalq({
          ...
          if(!update) {
              ...
              harmonize_is_ok_type(output_name
                                 , x.length = 1
                                 , type = "character"
                                 , allow.na = FALSE, allow.null = FALSE)
