...
nstandr package standardizes organizational names mainly using procedures described in Thoma et al. (2010) and Magerman, Looy, Bart, & Song (2006) but not only.  This is work in progress. Please, file an issues or suggestion if you have any.  The main function is [standardize()].


Standardizes organizational names using steps described in Thoma et al. (2010) and Magerman, Looy, Bart, & Song (2006) and more.
...
|------------+---------+------------------------------------------------------------------------|
| [[https://github.com/markvanderloo/tinytest/blob/master/pkg/README.md][tinytest]]   |   1.3.1 | package developlent (unit testing)                                     |
| [[https://cran.r-project.org/web/packages/fastmatch/index.html][fastmatch]]  |   1.1.3 | can speed things up a bit                                              |
| [[https://rstudio.github.io/htmltools/index.html][htmltools]]  |   0.5.4 | used for escaping html in procedures desctiptions before visualization |
| [[http://rich-iannone.github.io/DiagrammeR/docs.html][DiagrammeR]] |   1.0.9 | needed for visualizing procedures lists                                |
#+TBLFM: $2='(org-sbe get-package-vesion (pkg $$1))


#+caption: Required packages (Imports)
#+name: ob-r-pkg-imports
| name       | version | comment                                                   |
|------------+---------+-----------------------------------------------------------|
| data.table |  1.14.2 | fast data.frames, used as main input and output data type |
| stringi    |   1.7.6 | fast string manipulations                                 |
| xml2       |   1.3.2 |                                                           |
| checkmate  |   2.0.0 | function arguments checker, ensures stability             |
#+TBLFM: $2='(org-sbe get-package-vesion (pkg $$1))
...
| lubridate      |         |                               |
| [[https://moodymudskipper.github.io/boomer][boomer]]         |         |                               |
#+TBLFM: $2='(org-sbe get-package-vesion (pkg $$1))
...
#+BEGIN_SRC yaml :tangle info.yml :noweb yes
  ...
  ref: nstandr
  lang: R
  title: <<get-org-file-key(key = "title")>>
  author: <<get-org-file-key(key = "author")>>
  date: <<get-org-file-key(key = "date")>>
  version: <<get-org-file-key(key = "r_package_version")>>
  url: <<get-org-file-key(key = "r_package_url")>>
  publisher: GitHub
  commit:
  implements:
...
#+name: save-rds-procedures-list
#+begin_src R :tangle no :noweb yes
  data.name <- ls()
  data <- get(data.name)

  data.dir <- "data"
  rda.filename <- "sysdata"
  rda.dir <- "./R"
  ...
  ## make a path for saving .rds
  data.path <- file.path(data.dir, paste0(data.name, ".rds"))

  ## just in case if there is not directory
  dir.create(rda.dir, recursive = TRUE, showWarnings = FALSE)

  ## make a path for saving .rda
  rda.path  <- file.path(rda.dir, paste0(rda.filename, ".rda"))
...
#+name: nstandr-inject-get-dots-src-old
#+BEGIN_SRC emacs-lisp :tangle no
  (save-window-excursion
    (let* ((dots-get-dots-block-name "get_dots")
...
#+name: nstandr-inject-get-dots-src
#+BEGIN_SRC emacs-lisp :tangle no
    (save-window-excursion
      (let* ((dots-r-pkg-path "~/org/research/dots/dots.src.org")
             (dots-r-pkg-buffer (find-file dots-r-pkg-path))
             (dots-r-pkg-src-block "get_dots")
             ;; this is used as condition in 'bind-get-dots' call in dots.src.org
             (tangling-from-nstandr-R-package t)
             ;; turn off babel prompts
             (org-confirm-babel-evaluate nil)
             ;; get src
             (info (with-current-buffer dots-r-pkg-buffer
                     (org-with-wide-buffer (org-babel-lob--src-info dots-r-pkg-src-block)))))
        (when info
          (org-babel-expand-noweb-references info dots-r-pkg-buffer))))
#+END_SRC

#+BEGIN_SRC R :tangle R/0_get_dots.r :noweb yes
  <<nstandr-inject-get-dots-src()>>
#+END_SRC
...
# Table is not used anymore the bellow now is the representation

#+name: nstandr_default_procedures_list
#+BEGIN_SRC R :tangle no :noweb yes :session *R-make-nstandr_default_procedures_list*
  ...
  <<save-rds-procedures-list>>
#+END_SRC

#+call: nstandr_default_procedures_list()
...
#+BEGIN_SRC R :tangle inst/tinytest/test_browse_dot_graph.r
  ...
  expect_equal(
      ...
      make_dot_nodes() |>
      paste0(collapse = "\n")
    , "node_1 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"><TR><TD BGCOLOR=\"#bdc3c7\">this is a</TD></TR></TABLE>>];\nnode_2 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"></TABLE>>];\nsubgraph cluster_3 {\n    label=\"c\";\n    bgcolor=\"#ecf0f1\";\n    fontname=\"Helvetica\"\n    node_3_1 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"></TABLE>>];\n    node_3_2 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"></TABLE>>];\n    node_3_3 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"></TABLE>>];\n}\nnode_4 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"></TABLE>>];\nsubgraph cluster_5 {\n    label=\"f\";\n    bgcolor=\"#ecf0f1\";\n    fontname=\"Helvetica\"\n    node_5_1 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"></TABLE>>];\n    node_5_2 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"></TABLE>>];\n    node_5_3 [label = <<TABLE BGCOLOR=\"white\" BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\"></TABLE>>];\n}")
     ...
     is.null()) {
      ...
      expect_equal(
        ...
        , c("<?xml version=\"1.0\" encoding=\"UTF-8\"?>", "<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" width=\"69\" height=\"507\" viewBox=\"0 0 69 507\">", 
            ...
            "<path fill-rule=\"nonzero\" fill=\"rgb(74.117647%, 76.470588%, 78.039216%)\" fill-opacity=\"1\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M 0.5 -477.5 L 0.5 -498.5 L 61.5 -498.5 L 61.5 -477.5 Z M 0.5 -477.5 \" transform=\"matrix(1, 0, 0, 1, 4, 503)\"/>", 
            "<g fill=\"rgb(0%, 0%, 0%)\" fill-opacity=\"1\">", "<use xlink:href=\"#glyph-0-3\" x=\"7.5\" y=\"18.699219\"/>", 
            "<use xlink:href=\"#glyph-0-4\" x=\"12.75\" y=\"18.699219\"/>", 
            "<use xlink:href=\"#glyph-0-5\" x=\"21.75\" y=\"18.699219\"/>", 
            "<use xlink:href=\"#glyph-0-6\" x=\"25.5\" y=\"18.699219\"/>", 
            "<use xlink:href=\"#glyph-0-7\" x=\"33\" y=\"18.699219\"/>", 
            "<use xlink:href=\"#glyph-0-5\" x=\"37.5\" y=\"18.699219\"/>", 
            "<use xlink:href=\"#glyph-0-6\" x=\"41.25\" y=\"18.699219\"/>", 
            "<use xlink:href=\"#glyph-0-7\" x=\"48.75\" y=\"18.699219\"/>", 
            "<use xlink:href=\"#glyph-0-8\" x=\"53.25\" y=\"18.699219\"/>", 
            "</g>", "<path fill-rule=\"nonzero\" fill=\"rgb(100%, 100%, 100%)\" fill-opacity=\"1\" d=\"M 28 82 L 28 61 L 41 61 L 41 82 Z M 28 82 \"/>", 
            "<path fill-rule=\"nonzero\" fill=\"rgb(74.117647%, 76.470588%, 78.039216%)\" fill-opacity=\"1\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M 24.5 -420.5 L 24.5 -441.5 L 37.5 -441.5 L 37.5 -420.5 Z M 24.5 -420.5 \" transform=\"matrix(1, 0, 0, 1, 4, 503)\"/>", 
            "<g fill=\"rgb(0%, 0%, 0%)\" fill-opacity=\"1\">", "<use xlink:href=\"#glyph-1-1\" x=\"31.5\" y=\"75.699219\"/>", 
            "</g>", "<path fill=\"none\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M 30.5 -478.417969 C 30.5 -471.53125 30.5 -461.796875 30.5 -453.0625 \" transform=\"matrix(1, 0, 0, 1, 4, 503)\"/>", 
            "<path fill-rule=\"nonzero\" fill=\"rgb(0%, 0%, 0%)\" fill-opacity=\"1\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M 34 -453.292969 L 30.5 -443.292969 L 27 -453.292969 Z M 34 -453.292969 \" transform=\"matrix(1, 0, 0, 1, 4, 503)\"/>", 
            "<path fill-rule=\"nonzero\" fill=\"rgb(100%, 100%, 100%)\" fill-opacity=\"1\" d=\"M 28 142 L 28 121 L 41 121 L 41 142 Z M 28 142 \"/>", 
            "<path fill-rule=\"nonzero\" fill=\"rgb(74.117647%, 76.470588%, 78.039216%)\" fill-opacity=\"1\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M 24.5 -360.5 L 24.5 -381.5 L 37.5 -381.5 L 37.5 -360.5 Z M 24.5 -360.5 \" transform=\"matrix(1, 0, 0, 1, 4, 503)\"/>", 
            "<g fill=\"rgb(0%, 0%, 0%)\" fill-opacity=\"1\">", "<use xlink:href=\"#glyph-1-2\" x=\"31.5\" y=\"135.699219\"/>", 
            ...
            "<path fill-rule=\"nonzero\" fill=\"rgb(0%, 0%, 0%)\" fill-opacity=\"1\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M
)\" fill-opacity=\"1\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M 24.51875 -15.487813 L 24.51875 -36.480781 L 37.510625 -36.480781 L 37.510625 -15.487813 Z M 24.51875 -15.487813 \" transform=\"matrix(0.0986193, 0, 0, 0.0986193, 0.394477, 49.605523)\"/>", 
  "<g fill=\"rgb(0%, 0%, 0%)\" fill-opacity=\"1\">", "<use xlink:href=\"#glyph-1-8\" x=\"3.105469\" y=\"47.405834\"/>", 
  "</g>", "<path fill=\"none\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M 30.499766 -73.436328 C 30.499766 -66.544297 30.499766 -56.800391 30.499766 -48.046719 \" transform=\"matrix(0.0986193, 0, 0, 0.0986193, 0.394477, 49.605523)\"/>", 
  "<path fill-rule=\"nonzero\" fill=\"rgb(0%, 0%, 0%)\" fill-opacity=\"1\" stroke-width=\"1\" stroke-linecap=\"butt\" stroke-linejoin=\"miter\" stroke=\"rgb(0%, 0%, 0%)\" stroke-opacity=\"1\" stroke-miterlimit=\"10\" d=\"M 33.985391 -48.284375 L 30.499766 -38.302813 L 27.014141 -48.284375 Z M 33.985391 -48.284375 \" transform=\"matrix(0.0986193, 0, 0, 0.0986193, 0.394477, 49.605523)\"/>", 
  "</svg>"))
...
#+name: magerman_patterns_detect_comma_and_period_irregularities
...
#+call: make-rds-patterns-table(data = magerman_patterns_detect_comma_and_period_irregularities, data.name = "magerman_patterns_detect_comma_and_period_irregularities")
...
#+call: make-rds-patterns-table(data = magerman_legal_forms_list, data.name = "magerman_legal_forms_list"))

#+NAME: magerman_legal_forms_list
...
#+name: nstandr_magerman_procedures_list
#+BEGIN_SRC R :tangle no :noweb yes :session *R-make-magerman_default_procedures_list*
  nstandr_magerman_procedures_list <- 
      list("Prepossessing" =
         ...
         , "Characters" =
               list("Removing HTML codes" = 'magerman_remove_html_codes'
                  ...
                  , "Replacing SGML coded characters" = 'magerman_replace_sgml_characters'                                             
                  , "Replacing proprietary characters" = 'magerman_replace_proprietary_characters'
                  , "Detecting Umlauts" = call('magerman_detect_umlaut', output_codes_col_name = 'magerman_umlaut')  # sets code names with umlaut for umlaut standardization later
                  , "Replacing accented characters" = 'magerman_replace_accented_characters')
         , "Punctuation" = list("Removing special characters" = 'magerman_remove_special_characters'                                           
                              , "Fixing quotation irregularities" = 'magerman_remove_double_quotation_marks_irregularities'                        
                              , "Removing double quotations" = 'magerman_remove_double_quotation_marks_beginning_end'                         
                              , "Removing non alphanumeric characters (1)" = 'magerman_remove_non_alphanumeric_at_the_beginning'                            
                              , "Removing non alphanumeric characters (2)" = 'magerman_remove_non_alphanumeric_at_the_end'                                  
                              , "Fixing comma and period irregularities" = 'magerman_replace_comma_period_irregularities')
         , "Legal forms" = list("Detecting legal form" = call('magerman_detect_legal_form', output_codes_col_name = 'legal_form')
                               ,"Removing legal form" = 'magerman_remove_legal_form_and_clean')
         , "Common words" = list( "Removing common words" = 'magerman_remove_common_words'                                                 
                               , "Fixing spelling variations" = 'magerman_replace_spelling_variation'                                          
         ...
         , "Umlaut standardization" = list("Fixing umlaut variations" = call('magerman_replace_umlaut', has_umlaut_col = 'magerman_umlaut', progress = FALSE)))

  <<save-rds-procedures-list>>
#+END_SRC

#+call: nstandr_magerman_procedures_list()
...
#+name: nstandr_cookburn_procedures_list
#+BEGIN_SRC R :tangle no :noweb yes :session *R-make-nstandr_cookburn_procedures_list*
  nstandr_cookburn_procedures_list <-
      list("Prepossessing" = list("Cleaning spaces"  = call('standardize_squish_spaces', wrap_in_spaces = TRUE)
                                , "Upper casing"  = 'standardize_toupper')
         , "Standartization" = list("Special removals and recoding for USPTO names"  = 'cockburn_detect_uspto'                          
...
#+call: nstandr_cookburn_procedures_list()
