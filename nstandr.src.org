-*- mode: org; eval: (org-babel-lob-ingest (file-name-concat (file-name-directory (locate-library "ob-r-pkg")) ob-r-pkg-src-file)) -*-
...
#+BEGIN_SRC R :tangle no
  make_dot_node <- function(procedure, node) {
      obj <- get0(procedure, ifnotfound = NULL)
      if(!is.null(obj)) {
          for(tag in c("title"
                     , "example"
                     , "ref"
                     , "pp")) {
              assign(tag
                   , attr(obj, paste0("nstandr_procedure_", tag)) |>
                     htmltools::htmlEscape())
          }
      }
      paste0("node_", node , " ["
           , "label = " , if(!is.null(obj) && length(title) != 0 && title != "") {
                              paste0(
                                  "<<TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\">"
                                , "<TR><TD BGCOLOR=\"lightblue\">", title, "</TD></TR>"
                                , if(length(example) != 0 && example != "") {
                                      paste0("<TR><TD>", example, "</TD></TR>")
                                  }
                                , "</TABLE>>")
                          } else {
                              paste0("\"", procedure,"\"")
  ...
  ## make_dot_node(1, "magerman_replace_accented_characters")
  ...
  make_dot_nodes <- function(procedures_list, node_prefix = NULL) {
      ...
      mapply(\(p, name, n) {
          if(is.list(p) && !is.pairlist(p)) {
              paste("subgraph cluster {"
                   , "style=invisible;"
                   , "subgraph cluster_", n," {"
                   ...
                   , "color=lightgrey;"
                   , "node [color=white];"
                   , make_dot_nodes(p, node_prefix = paste0(n, "_"))
                   , "}", "}"
                   , sep = "\n")
          } else {
              make_dot_node(p[[1]], node = paste0(node_prefix, n))
          }
      }
    , p = procedures_list
  ...
  ## nstandr:::magerman_procedures_list |>
  ##     make_dot_nodes()
  ...
  make_dot_edges <- function(procedures_list, prefix = NULL) {
      mapply(
          \(p, pp, n, nn) {
              if(is.list(pp) && !is.pairlist(pp)) {
                  paste(
                      paste0("node_", n, " -> ", "node_", n, "_1"
                           , " [lhead = cluster_", n,"];")
                    , make_dot_edges(pp, prefix = paste0(n, "_"))  
                    , paste0("node_", n, "_", length(pp), " -> ", "node_", nn
                           , " [ltail = cluster_", n,"];")
                      , sep = "\n")
              } else {
                  paste0("node_", n, " -> ", "node_", nn, ";")
              }
          }
        , p = procedures_list[-length(procedures_list)]
        , pp = procedures_list[-1]
        , n = paste0(prefix, seq(length(procedures_list) - 1))
        , nn = paste0(prefix, seq(2, length(procedures_list)))
      ) |>
      paste(collapse = "\n")
  }
  ...
  make_dot_graph <- function(procedures_list) {
      paste("digraph nstantr_procedures {" ,
          ...
          , make_dot_nodes(procedures_list),
          , make_dot_edges(procedures_list), 
          , "}"
          , sep = "\n\n")
  }
